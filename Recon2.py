...
if uploaded_files:
    ...
    preview_img = thumbs[0].convert('RGBA')
    # Вставка водяного знака в правый нижний угол
    if watermark_files:
        wm = watermark_img.copy()
        # Масштабируем водяной знак под размер фото
        scale = min(preview_img.size[0] // 4 / wm.size[0], preview_img.size[1] // 4 / wm.size[1])
        wm = wm.resize((int(wm.size[0]*scale), int(wm.size[1]*scale)), Image.ANTIALIAS)
        pos = (preview_img.size[0] - wm.size[0] - 10, preview_img.size[1] - wm.size[1] - 10)
    ...
    if st.button('Обработать и скачать архив'):
        ...
        with zipfile.ZipFile(zip_buffer, 'w') as zipf:
            for idx, file in enumerate(uploaded_files):
                ...
                wm = watermark_img.copy()
                scale = min(img.size[0] // 4 / wm.size[0], img.size[1] // 4 / wm.size[1])
                wm = wm.resize((int(wm.size[0]*scale), int(wm.size[1]*scale)), Image.ANTIALIAS)
                pos = (img.size[0] - wm.size[0] - 10, img.size[1] - wm.size[1] - 10)
                img.paste(wm, pos, wm)
